...
const GroupCallMain = () => {
  ...
  const enter = useCallback(async (roomId: string, skipDialog: boolean) => {
    try {
      ...
      RoomRequest.findById(roomId, function (data: any) {
        if (data.httpStatus != 'OK') {
          toast.error(<ErrorMessage message={data.message} />, { autoClose: 2000 });
          return null;
        }
  ...
  const createRoom = useCallback(() => {
    sbCalls.createRoom({ roomType: sbCalls.RoomType.SMALL_ROOM_FOR_VIDEO })
      .then(room => {
        ...
        ///////
        RoomRequest.createRoom(room.roomId, function (data: any) {
          if (data.httpStatus != 'OK')
            alert(data.message);
          else
            return room.enter({
